...
const SPAWN = new Vector3(0, EYE_HEIGHT, -12)
...
export function createNewProjectScene(
  ...
  const camera = new UniversalCamera('stage-camera', SPAWN.clone(), scene)
  ...
  let interaction: string | null = 'WASD/arrows move · drag to look · R resets'
  ...
  return {
    ...
    update: (deltaSeconds) => {
      if (mode === 'play' && context.input.consume('KeyR')) {
        ...
        motionPhase = 0
        motionState = 'idle'
        interaction = 'camera returned to the scaffold origin'
      }
      ...
      if (dt > 0) {
        ...
        motionPhase = (motionPhase + distance / GRID_STEP) % 1
        previousPosition.copyFrom(camera.position)
      }
    },
