...
import type { Rides, Shape, Shared } from './types.ts'
...
export function makeDescent(c: Shared): Shape {
  ...
  const DS = ((): Rides | null => {
    ...
    for (var di = 0; di < S.placed.length; di++) {
      if (S.placed[di]?.rides === undefined) continue
      return {
  ...
  if (D0) {
    ...
    if (D0.clouds) {
      var DC = D0.clouds, CLD = S.layers[DC.layer]
   ...
   */
  function persp(A) { return D0.zNear / Math.max(D0.zNear * 0.28, A + D0.zNear) }

  function descentRow(f) { return D0.horizonRow + (D0.holdY - D0.horizonRow) * f }
